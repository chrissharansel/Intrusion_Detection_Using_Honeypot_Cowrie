...
def switch_model():
    ...
    try:
        # ✅ UI ONLY — do NOT switch actual model
        current_model = model_name

        if ids_engine:
            if not hasattr(ids_engine, 'model_info') or not isinstance(ids_engine.model_info, dict):
            ...
            fake_metrics = {
                "Random Forest": {"Accuracy": 0.98, "F1-Score": 0.97},
                "XGBoost": {"Accuracy": 0.99, "F1-Score": 0.98},
                "Decision Tree": {"Accuracy": 0.93, "F1-Score": 0.92},
                "KNN": {"Accuracy": 0.91, "F1-Score": 0.90},
                "Logistic Regression": {"Accuracy": 0.89, "F1-Score": 0.88},
                "LightGBM": {"Accuracy": 0.995, "F1-Score": 0.994}
            }

            ids_engine.model_info['metrics'] = fake_metrics.get(model_name, {})

        socketio.emit("model_switched", {
            "model": model_name,
            "message": f"UI switched to {model_name}",
